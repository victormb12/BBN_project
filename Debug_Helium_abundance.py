...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan  7 12:44:01 2021
...
import Debug_Helium_Functions as He
import time
...
# Neff=np.array([3])
# xi_nu=np.array([0])
# He.plot_Boltz(T_range=T_range,Neff=Neff,xi_nu=xi_nu,n_over_p=True)
...
etas=np.logspace(-10,-9,100)
Neff=np.array([3])
xi_nu=np.array([0])
He.plot_He(etas,xi_nu=xi_nu,Neff=Neff,T_d=1)
...
Neff=np.array([3])
xi_nu=np.array([0])
Y=He.Y_He_fin(etas, Neff, xi_nu,T_d=0.073,verbose=True)
print('The Helium Abundance is: Y_He={:.6g}'.format(Y[(Neff[0],xi_nu[0])]['$Y_{He}$'].iloc[0]))
...
# eta1=np.array([5e-10])#np.logspace(-10,-9,100)
# Y1=0.245
