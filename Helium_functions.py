...
def n_to_p_int(epsilon,z_gamma,sign=1,xi_nu=0):
    ...
    T=0.511/z_gamma
    # print(T_over_nu(T))
    K=MeV_to_1_over_s(1/(1.636*879.5),b=0)
    q=1.2933/0.511
...
def rate_spline(z_range,dN_eff=0,xi_nu=0):
    filename_Hz_np='gamma_H_z_np'
    filename_Hz_pn='gamma_H_z_pn'
    gamma_H_z_np(filename_Hz_np,z_range=z_range,dN_eff=dN_eff,xi_nu=xi_nu)
    gamma_H_z_pn(filename_Hz_pn,z_range=z_range,dN_eff=dN_eff,xi_nu=xi_nu)
...
def BBN_func2(zc,eta,Xn,g_eff):
    ...
    func=const*dzdt*zc**-3.5*np.exp(zc)*Xn(z)*(1-Xn(z))*interp(zc)*0.04
    return func-1
...
#     geffs= ge.g_eff_s_tot(T, return_spline=True)
#     dgeffs= lambda T: geffs.derivative(1)(T)
#     f=0.03*(1.18e13*eta**2/np.sqrt(g_eff(2.23/zc))*(zc)**(-17/6)*np.exp(-1.44*(zc)**(1/3))*np.exp(zc)*Xn(z)*(1-Xn(z))*(1+1/3*T/geffs(T)*dgeffs(T)))-1
#     return f
...
def Y_He_control(eta,tau_n=880.3,dN_eff=0,xi_nu=0,steigman=False,Td=1):
    if steigman:
...
def Delta_Y_P_control(xi_nu,dN_eff,deta,dTau_n,order=1,return_absolute=False):
    eta=6.1e-10
    tau_n=879.5
    N=3
    Yp_0=0.24709
    filename='Coefficients_expansion_Pytrou.csv'
    coefficients=pd.read_csv(filename, delimiter=';',dtype={'coefficient':str})
    coefficients=coefficients[['coefficient','Yp']]
    values=[str(i)+str(j)+str(k) for i,j,k in list(itertools.product(range(0,order+1),range(0,order+1),range(0,order+1)))]
    coefficients=coefficients.loc[coefficients['coefficient'].isin(values)]
    func=lambda row: row[1]*(deta/eta)**float(row[0][0])*(dN_eff/N)**float(row[0][1])*(dTau_n/tau_n)**float(row[0][2])
    coefficients['product']=coefficients.apply(func,axis=1)
    suma=coefficients['product'].sum()
    if return_absolute:
        Yp=Yp_0*(1+suma-0.96*xi_nu)
        return Yp
    return suma-0.96*xi_nu

def Delta_D_H_control(xi_nu,dN_eff,deta,dTau_n,order=1,return_absolute=False):
    eta=6.1e-10
    tau_n=879.5
    N=3
    D_H_0=2.459
    filename='Coefficients_expansion_Pytrou.csv'
    coefficients=pd.read_csv(filename, delimiter=';',dtype={'coefficient':str})
    coefficients=coefficients[['coefficient','D/H']]
    values=[str(i)+str(j)+str(k) for i,j,k in list(itertools.product(range(0,order+1),range(0,order+1),range(0,order+1)))]
    coefficients=coefficients.loc[coefficients['coefficient'].isin(values)]
    func=lambda row: row[1]*(deta/eta)**float(row[0][0])*(dN_eff/N)**float(row[0][1])*(dTau_n/tau_n)**float(row[0][2])
    coefficients['product']=coefficients.apply(func,axis=1)
    suma=coefficients['product'].sum()
    if return_absolute:
        D_H=(1+suma-0.53*xi_nu)*D_H_0
        return D_H
    return suma-0.53*xi_nu
...
def plot_He(eta,dN_eff=0,xi_nu=0,deviation=False,compare=True,T_d=1,plot=True):
    if type(dN_eff)!=type(np.array([1])):
        dN_eff=np.array([dN_eff])
    if type(xi_nu)!=type(np.array([1])):
        xi_nu=np.array([xi_nu])
    Y_dict=Y_He_fin(eta,dN_eff,xi_nu,T_d=T_d)
    # if deviation:
    ...
    if plot:
        fig,ax=plt.subplots(1,1)
        Y_dict[(dN_eff[0],xi_nu[0])].plot(ax=ax,label=r'$Neff={:.3g}, \xi_\nu={:.3g}$'.format(dN_eff[0],xi_nu[0]))    
        ax.set_xlabel(r'$\eta$')
        ax.set_ylabel(r'$Y_{He}$')
        if compare:
            ax.plot(eta,Y_He_control(eta,xi_nu=xi_nu,dN_eff=dN_eff),label='PDG review')
        ax.plot(eta,[0.245]*len((eta)))
        ax.fill_between(eta,[0.245-0.003]*len(eta),[0.245+0.003]*len(eta),alpha=0.5)
        ax.fill_betweenx(ax.get_ylim(),[5.8e-10]*2,[6.5e-10]*2,alpha=0.5)
        plt.semilogx()
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
    plot_Y=Y_dict[(dN_eff[0],xi_nu[0])][r'$Y_{He}$'].to_numpy()
    return plot_Y

def plot_Td(eta,xi_nu=0,dN_eff=0,compare=False):
    return_spline=True
    m=0.511
...
def Y_He_fin(etas=np.array([6.1e-10]),dN_effs=np.array([0]),xi_nus=np.array([0]),verbose=False,T_d=1):
    start_time=time.time()
...
def Y_He_fin_control(detas=np.array([0]),dN_effs=np.array([0]),xi_nus=np.array([0]),order=1):
    dTau_n=0
    start_time=time.time()
    if type(detas)!=type(np.array([1])):
    ...
    Y_dict={}
    for dN_eff,xi_nu in itertools.product(dN_effs,xi_nus):
        Y_dict[(dN_eff,xi_nu)]=pd.DataFrame(Delta_Y_P_control(xi_nu,dN_eff,detas,dTau_n,return_absolute=True),index=detas, columns=[r'$Y_{He}$'])
    print("--- %s seconds ---" % (time.time() - start_time))
    return Y_dict  

def D_H_fin_control(detas=np.array([0]),dN_effs=np.array([0]),xi_nus=np.array([0]),order=1):
    ...
    Y_dict={}
    for dN_eff,xi_nu in itertools.product(dN_effs,xi_nus):
        Y_dict[(dN_eff,xi_nu)]=pd.DataFrame(Delta_D_H_control(xi_nu,dN_eff,detas,dTau_n,return_absolute=True),index=detas, columns=[r'$\frac{D}{H}$'])
    print("--- %s seconds ---" % (time.time() - start_time))
    return Y_dict  

def N_v_xi(eta,Y,dN_effs,xi_nus,step=100,epsilon=0.003,create_dict=False,plot=True,error=True,ax=None):
    filename=filename='N_vs_xi_control_{:}_{:}_{:}'.format(eta,Y,epsilon)
    if create_dict:    
        if type(eta)!=type(np.array([1])):
            eta=np.array([eta])
        Y_dict=Y_He_fin(eta,dN_effs,xi_nus)
        dict_to_pkl(Y_dict,filename)
    else:
        try:
            Y_dict=dict_from_pkl(filename)
        except FileNotFoundError:
            print('No file was found under that name, but a new one is being created.')
            Y_dict=Y_He_fin(eta,dN_effs,xi_nus)
            dict_to_pkl(Y_dict,filename)
    plot_xi=[]
    plot_dN_eff=[]
    for dN_eff,xi_nu in Y_dict.keys():
        if Y-epsilon <= Y_dict[(dN_eff,xi_nu)].iloc[0][0]<=Y+epsilon:
            plot_xi.append(xi_nu)
            plot_dN_eff.append(dN_eff)
    plot_xi=np.array(plot_xi)
    plot_dN_eff=np.array(plot_dN_eff)
    if error: 
        dN_max=[]
        dN_min=[]
        xi_nu_fill=[]
        for xi in plot_xi:
            if xi not in xi_nu_fill:
                Neffs=plot_dN_eff[np.where(plot_xi==xi)]
                dN_max.append(max(Neffs))
                dN_min.append(min(Neffs))
                xi_nu_fill.append(xi)
        
    if plot:
        if not ax:
            fig,ax=plt.subplots(1,1)
        ax.set_xlabel(r'$\xi_\nu$')
        ax.set_ylabel(r'$\Delta N_{eff}$')
        ax.plot(plot_xi,plot_dN_eff,label=r'$\eta={0:.3g},Y={1:.3g}$'.format(eta[0],Y))
        if error:
            ax.fill_between(xi_nu_fill,dN_min,dN_max,alpha=0.5)
        ax.legend()
        ax.grid(True)
        plt.tight_layout()
                
    return plot_dN_eff,plot_xi
        
def N_v_xi_control(eta,Y,dN_effs,xi_nus,epsilon=0.003,plot=True,error=True,order=1,ax=None):
    Y_dict=Y_He_fin_control(etas=eta,dN_effs=dN_effs,xi_nus=xi_nus,order=order)
    plot_xi=[]
    plot_dN_eff=[]
    for dN_eff,xi_nu in Y_dict.keys():
        if Y-epsilon <= Y_dict[(dN_eff,xi_nu)].iloc[0][0]<=Y+epsilon:
...
def N_v_xi_deuterium(eta,Y,dN_effs,xi_nus,epsilon=0.036,plot=True,error=True,order=1,ax=None):
    Y_dict=D_H_fin_control(detas=eta,dN_effs=dN_effs,xi_nus=xi_nus,order=order)
    plot_xi=[]
    plot_dN_eff=[]
    for dN_eff,xi_nu in Y_dict.keys():
        if Y-epsilon <= Y_dict[(dN_eff,xi_nu)].iloc[0][0]<=Y+epsilon:
...
def Y_v_N(eta,xi_nu,dN_effs,create_dict=False,plot=True,ax=None):
    filename='Y_vs_N_{:}_{:}'.format(eta,xi_nu)
    if create_dict:    
        if type(eta)!=type(np.array([1])):
            eta=np.array([eta])
        if type(xi_nu)!=type(np.array([1])):
            xi_nu=np.array([xi_nu])
        Y_dict=Y_He_fin(eta,dN_effs,xi_nu)
        dict_to_pkl(Y_dict,filename)
    else:
        try:
            Y_dict=dict_from_pkl(filename)
    ...
    plot_dN_eff=[]
    for dN_eff,xi_nu in Y_dict.keys():
        plot_Y.append(Y_dict[(dN_eff,xi_nu)].iloc[0][0])
        plot_dN_eff.append(dN_eff)
    if plot:
        if not ax:
            fig,ax=plt.subplots(1,1)
        ax.set_xlabel(r'$\Delta N_{eff}$')
        ax.set_ylabel(r'$Y_{He}(\Delta N_{eff})$')
        ax.plot(plot_dN_eff,plot_Y,label=r'$\eta={0:.3g},\xi_\nu={1:.3g}$'.format(eta[0],xi_nu))
        ax.fill_between(plot_dN_eff,[0.245-0.003]*len(plot_dN_eff),[0.245+0.003]*len(plot_dN_eff),alpha=0.5)
        ax.legend()
        ax.grid(True)
        ax.set_ylim(0.22,0.27)
        plt.tight_layout()
    return np.array(plot_dN_eff),np.array(plot_Y)
   
def Y_v_N_control(eta,xi_nu,dN_effs,create_dict=False,plot=True,order=1,ax=None):
    Y_dict=Y_He_fin_control(etas=eta,dN_effs=dN_effs,xi_nus=xi_nu,order=order)
    plot_Y=[]
    plot_dN_eff=[]
    for dN_eff,xi_nu in Y_dict.keys():
        plot_Y.append(Y_dict[(dN_eff,xi_nu)].iloc[0][0])
...
def Y_v_xi(eta,dN_eff,xi_nus,create_dict=False,plot=True):
    filename='Y_vs_xi_{:}_{:}'.format(eta,dN_eff)
    if create_dict:    
        if type(eta)!=type(np.array([1])):
            eta=np.array([eta])
        if type(dN_eff)!=type(np.array([1])):
            dN_eff=np.array([dN_eff])
        Y_dict=Y_He_fin(eta,dN_eff,xi_nus)
        dict_to_pkl(Y_dict,filename)
    else:
        try:
            Y_dict=dict_from_pkl(filename)
    ...
    if plot:
        ...
        ax.plot(plot_xi,plot_Y,label=r'$\eta={0:.3g},\Delta Neff={1:.3g}$'.format(eta[0],dN_eff))
        ax.legend()
        ax.grid(True)
        plt.tight_layout()
    return np.array(plot_xi),np.array(plot_Y)

def Y_v_xi_control(eta,dN_eff,xi_nu,create_dict=False,plot=True,order=1):
    Y_dict=Y_He_fin_control(etas=eta,dN_effs=dN_eff,xi_nus=xi_nu,order=order)
    plot_xi=[]
    plot_Y=[]
    for dN_eff,xi_nu in Y_dict.keys():
        plot_Y.append(Y_dict[(dN_eff,xi_nu)].iloc[0][0])
...
def eta_v_xi(Y,dN_eff,xi_nus,etas,epsilon=0.001,create_dict=False,plot=True):
    filename='eta_vs_xi_2'
    if create_dict:    
        if type(dN_eff)!=type(np.array([1])):
            dN_eff=np.array([dN_eff])
        Y_dict=Y_He_fin(etas,dN_eff,xi_nus)
        dict_to_pkl(Y_dict,filename)
    else:
        Y_dict=dict_from_pkl(filename)
    plot_eta=[]
    ...
    if plot:
        ...
        ax.set_xlabel(r'$\eta$')
        ax.set_ylabel(r'$\xi_{\nu}$')
        ax.plot(plot_eta,plot_xi,label=r'$Y={0:.3g},\Delta Neff={1:.3g}$'.format(Y,dN_eff))
        ax.legend()
        ax.grid(True)
        ax.semilogx()
        plt.tight_layout()
