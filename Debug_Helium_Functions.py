...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan  7 12:39:07 2021
...
import pickle
from scipy.interpolate import interp1d, Rbf,interp2d
...
def n_to_p_int(epsilon,z_gamma,sign=1,xi_nu=0):
    ...
    q=1.2933/0.511
    """
    HERE IT ONLY WORKS IF IT IS DIVIDED BY T OVER T NU, BUT IF I HAVE UNDERSTOOD EVERYTHING CORRECTLY THIS CANNOT BE THE CASE
    """
...
def gamma_pn_spline(filename,z_range=(-4,3),xi_nu=0,return_spline=False):
    z=np.logspace(z_range[0],z_range[1],100)
    func=gamma_p_to_n(z)
...
def gamma_H_z_pn(filename,z_range,Neff=3,xi_nu=0,return_spline=False):
    ...
    func=gamma_p_to_n(zi,xi_nu)/(1.66*np.sqrt(g_eff(0.511/zi))*(0.511)**2/1.221e22)*zi
    f2 = interp1d(zi, func, kind='cubic')
    if return_spline:
        return f2
    func_to_pkl(f2,filename)
    
def gamma_H_z_np(filename,z_range,Neff=3,xi_nu=0,return_spline=False):
    g_eff= lambda T: ge.g_eff_tot(T,ge.N_eff_SMC(T,N_nu=Neff,xi_nu=xi_nu))
    zi=np.logspace(np.log10(z_range[0])-1,np.log10(z_range[1])+1,1000)
    func=gamma_n_to_p(zi,xi_nu)/(1.66*np.sqrt(g_eff(0.511/zi))*(0.511)**2/1.22e22)*zi
    f2 = interp1d(zi, func, kind='cubic')
    if return_spline:
        return f2
    func_to_pkl(f2,filename)
...
def Y_He(eta,solved_Boltz,Neff=3,xi_nu=0,verbose=False,Td=1):
    Xn=solved_Boltz.sol
    if Td==1:
...
def solve_Boltz(T_range=(2,1e-2),Neff=3,xi_nu=0,create_spline=False,H_z_pn_rate=None,H_z_np_rate=None ,rtol=1e-6,atol=1e-20,y0=None):
    g_eff= lambda T: ge.g_eff_tot(T,ge.N_eff_SMC(T,N_nu=Neff,xi_nu=xi_nu))
...
def plot_Boltz(T_range=(2,1e-2),Neff=3,xi_nu=0,vline=False,step=1000,n_over_p=False):
    ...
    Boltzmann2=solve_Boltz(T_range,Neff=Neff,xi_nu=xi_nu,H_z_pn_rate=pn_rate,H_z_np_rate=np_rate)
    if n_over_p:
        ...
        n_over_p=Xn/(1-Xn)
        n_over_p2=interp1d(df.index.to_numpy(),df['y'].to_numpy(),kind='cubic')
        ax[0].set_ylim((0,1))
...
def plot_He(eta,Neff=3,xi_nu=0,deviation=False,compare=True,T_d=1):
    ...
    if compare:
        ax.plot(eta,Y_He_control(eta,xi_nu=xi_nu,Neff=Neff),label='Bernstein et al.')
...
def Y_He_fin(etas=np.array([6.1e-10]),Neffs=np.array([3]),xi_nus=np.array([0]),verbose=False,T_d=1):
    start_time=time.time()
    ...
    T_range=(2,1e-2)#in MeV
    z_range=(m/T_range[0],m/T_range[1])
    for Neff,xi_nu in itertools.product(Neffs,xi_nus):
        print(Neff,xi_nu)
        np_rate=gamma_H_z_np('',z_range,Neff=Neff,xi_nu=xi_nu, return_spline=return_spline)
        pn_rate=gamma_H_z_pn('',z_range,Neff=Neff,xi_nu=xi_nu, return_spline=return_spline)
        Xn=solve_Boltz(T_range,Neff=Neff,xi_nu=xi_nu,H_z_pn_rate=pn_rate,H_z_np_rate=np_rate)
        # boltzmann_dict[(Neff,xi_nu)]=Xn
        Y_dict[(Neff,xi_nu)]=pd.DataFrame(Y_He(etas,Xn.func_sol,Neff=Neff,xi_nu=xi_nu,Td=T_d).T,index=etas, columns=[r'$Y_{He}$'])
    print("--- %s seconds ---" % (time.time() - start_time))
    return Y_dict
